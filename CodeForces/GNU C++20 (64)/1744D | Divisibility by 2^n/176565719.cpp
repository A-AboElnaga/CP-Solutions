...
int main()
{
  ...
  {
    int n; cin >> n;
    int num_contr =0;        //array numbers contribution the the count of number of 2s in the product
    vector<int> indx_contr;
    for(int i =1; i <=n ; i++){
      int x; cin >> x;
      num_contr+= __builtin_ctz(x);
      if(!(i&1))
      indx_contr.push_back(__builtin_ctz(i));
    }
    if(num_contr>=n) cout << 0 << endl;
    else{
      sort(indx_contr.rbegin(),indx_contr.rend());
      long long max = 0;
      for( auto itr = indx_contr.begin(); itr != indx_contr.end();itr++){
        max+=*itr;
       *itr=max;
      }
      if(max+num_contr<n) cout << -1 <<endl;
      else{
          auto it = upper_bound(indx_contr.begin(),indx_contr.end(),n-num_contr-1);
          it++;
          cout << distance(indx_contr.begin(), it) << endl ; 
      }
    }
