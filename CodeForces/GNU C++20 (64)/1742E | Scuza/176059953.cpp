...
int main()
{
  ...
  while(t--){
    ...
    vector<pair<int/*val*/,int/*indx*/>> v2;
    int mx{};
    for(int i =1; i<=n;i++){
      cin >> v[i];
      if(v[i]>mx) v2.push_back(make_pair(v[i],i));
      mx=max(mx,(int)v[i]);
      v[i]+=v[i-1];
    }
    while(q--){
        int k; cin >>k;
        if (k>=mx){cout << v[n] <<' '; continue;}
        auto  rslt = upper_bound(v2.begin(),v2.end(),make_pair(k,n));
        cout <<v[rslt->second-1]<<' ';
    }
    cout << endl;
